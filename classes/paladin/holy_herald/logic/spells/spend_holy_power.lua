...
function FS.paladin_holy_herald.logic.spells.spend_holy_power(requireBA)
    ...
    -- Check if we recently cast Holy Prism and should be applying Dawnlight
    local apply_dawnlight = FS.paladin_holy_herald.variables.should_prioritize_dawnlight()
    
    -- Determine spending priority between different Holy Power spenders
    ...
    -- PRIORITY 1: When Dawnlight needs to be applied via Eternal Flame
    if apply_dawnlight and 
       FS.paladin_holy_herald.settings.use_eternal_flame() and 
       FS.paladin_holy_herald.settings.hp_prioritize_dawnlight() then
        
        -- Target selection for Dawnlight application
        local target
        if FS.paladin_holy_herald.settings.prioritize_self_dawnlight() then
            target = FS.variables.me
        else
            -- Get the best target for Dawnlight application
            for _, unit in ipairs(FS.modules.heal_engine.units) do
                if not FS.variables.buff_up(FS.paladin_holy_herald.auras.dawnlight, unit) then
                    if not target or
                       FS.api.unit_helper:get_health_percentage(unit) < 
                       FS.api.unit_helper:get_health_percentage(target) then
                        target = unit
                    end
                end
            end
            
            -- Fallback to self if no suitable target found
            if not target then
                target = FS.variables.me
            end
        end
        ...
        if target then
            FS.api.spell_queue:queue_spell_target(FS.paladin_holy_herald.spells.eternal_flame, target, 0) -- Priority 0 for Dawnlight application
            track_hp_spender()
            return true
        end
    end
    ...
    -- PRIORITY 3: Critical single-target healing with Word of Glory
    ...
    if critical_target then
        FS.api.spell_queue:queue_spell_target(FS.paladin_holy_herald.spells.word_of_glory, critical_target, 0) -- Priority 0 for critical healing
        track_hp_spender()
        return true
    end
    
    -- PRIORITY 4: Multi-target evaluation
    -- Count how many targets would benefit from LoD versus WoG/EF
    
    local lod_min_targets = FS.paladin_holy_herald.settings.lod_min_targets()
    local wog_threshold = FS.paladin_holy_herald.settings.wog_hp_threshold()
    ...
    if not (overcap_risk or imminent_overcap) then
        ...
        -- Only check if talent is learned and setting is enabled
        if FS.paladin_holy_herald.settings.use_eternal_flame() and
           FS.paladin_holy_herald.talents.eternal_flame then
        ...
        if FS.paladin_holy_herald.settings.use_eternal_flame() and
           FS.paladin_holy_herald.talents.eternal_flame then
            ...
            ef_efficiency = ef_efficiency + (ef_targets_need_refresh * 0.8)  -- Each HoT needing refresh is worth 0.8 points
    ...
    -- PRIORITY 5: Regular single-target Word of Glory
    if FS.paladin_holy_herald.logic.spells.word_of_glory() then
        track_hp_spender()
        return true
    end
    ...
    if enemy_target then
        ...
        if target_pos and self_pos and target_pos:dist_to(self_pos) <= 8 and
           FS.api.spell_helper:is_spell_queueable(FS.paladin_holy_herald.spells.shield_of_the_righteous, FS.variables.me, FS.variables.me, true, true) then
            ...
            local priority = (overcap_risk or imminent_overcap) and 0 or 2
            
            FS.api.spell_queue:queue_spell_target(FS.paladin_holy_herald.spells.shield_of_the_righteous, FS.variables.me, priority)
            track_hp_spender()
            return true
        end
    end
