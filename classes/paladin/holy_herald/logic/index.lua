...
function FS.paladin_holy_herald.logic.on_update()
    ...
    if FS.variables.me:is_casting_spell() then
        return
    end

    if FS.variables.me:is_channelling_spell() then
        return
    end
    ...
    if FS.paladin_holy_herald.logic.spells.blessing_of_the_seasons() then
        return
    end
