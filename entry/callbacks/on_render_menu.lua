                        ...
                        if error_count > 0 then
                            for i, error_info in ipairs(errors_for_display) do
                                ...
                                if error_info.status:find("Disabled") then
                                    error_text = "⛔ " ..
                                    error_info.component .. " (" .. error_info.count .. ") - " .. time_text
                                ...
                                error_button:render_as_selectable(error_text, err_color,
                            ...
                            if FS.menu.error_handler.selected_error then
                                ...
                                if details and #details.instances > 0 then
                                    ...
                                    core.menu.text("Error Details: " .. FS.menu.error_handler.selected_error):render(
                                    ...
                                    if latest.game_state then
                                        ...
                                        core.menu.text("  In Combat: " .. (latest.game_state.in_combat and "Yes" or "No"))
                                            :render()
                                    end
                                end
                            end
            ...
            if FS.menu.profiler then
                ...
                if FS.menu.profiler.tree:is_open() then
                    ...
                    if FS.menu.profiler.show_metrics:get_state() and FS.profiler.config.enabled then
                        ...
                        -- Sort options
                        FS.menu.profiler.sort_by:set_options({ "Total Time", "Count", "Average Time", "Max Time",
                            "Last Time" })
                        ...
                        if #metrics == 0 then
                            ...
                            core.menu.text(string.format("Memory Usage: %.2f MB", memory_stats.memory_mb)):render(
                            mem_color)
                            ...
                            -- Simple table header
                            local header_text =
                            "Name                        Count      Avg (ms)    Max (ms)    Total (ms)"
                            ...
                            if FS.menu.profiler.selected_metric then
                                ...
                                if metric and history and #history > 0 then
                                    ...
                                    -- Display details header
                                    core.menu.text("Metric Details: " .. selected_key):render(color.new(255, 255, 100,
                                        255))

                                    -- Basic stats
                                    core.menu.text(string.format("  Calls: %d", metric.count)):render()
                                    core.menu.text(string.format("  Avg Time: %.2f ms",
                                        metric.count > 0 and metric.total_time / metric.count or 0)):render()
                                    ...
                                    if time_ago < 60 then
                                        core.menu.text(string.format("  Last Call: %.1f seconds ago (%.2f ms)", time_ago,
                                            metric.last_time)):render()
                                    else
                                        core.menu.text(string.format("  Last Call: %.1f minutes ago (%.2f ms)",
                                            time_ago / 60, metric.last_time)):render()
                                    end
            ...
            if FS.menu.memory then
                ...
                if FS.menu.memory.tree:is_open() then
                    ...
                    if FS.menu.memory.show_pools:get_state() then
                        ...
                        if #leaks > 0 then
                            -- Memory leak warning
                            core.menu.text("⚠️ Potential Memory Leaks Detected (" .. #leaks .. ")"):render(color.new(255,
                                60, 60, 255))
                        ...
                        core.menu.text(string.format("Total Pooled Objects Memory: %.2f MB", total_mb)):render(color.new(
                        200, 200, 255, 255))

                        -- Display table header
                        local header_text =
                        "Pool Name                     Type        Size    In Use   Memory (KB)   Health"
                        core.menu.text(header_text):render()
                        ...
                        if #pool_stats == 0 then
                            ...
                            if FS.menu.memory.selected_pool then
                                ...
                                if selected_stats then
                                    ...
                                    -- Display details header
                                    core.menu.text("Pool Details: " .. selected_stats.name):render(color.new(255, 255,
                                        100, 255))

                                    -- Basic stats
                                    core.menu.text(string.format("  Object Type: %s", selected_stats.object_type))
                                        :render()
                                    core.menu.text(string.format("  Total Created: %d", selected_stats.created)):render()
                                    core.menu.text(string.format("  Currently In Use: %d", selected_stats.acquired))
                                        :render()
                                    core.menu.text(string.format("  Idle In Pool: %d", selected_stats.pool_size)):render()
                                    core.menu.text(string.format("  Total Recycled: %d", selected_stats.recycled))
                                        :render()
                                    ...
                                    core.menu.text(string.format("  Cache Hit Rate: %.1f%%",
                                        selected_stats.recycle_rate * 100)):render(efficiency_color)
                                    core.menu.text(string.format("  Reuse Efficiency: %.1f%%",
                                        selected_stats.efficiency_score * 100)):render(efficiency_color)
                                end
                            end
                        end
                    end
