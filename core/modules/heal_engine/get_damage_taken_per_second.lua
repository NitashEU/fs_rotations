...
--- Find the first index in health values with a timestamp >= start_time.
---@param values table[] List of health record tables with a 'time' field.
---@param start_time number The lower bound timestamp.
---@return number first_valid_index The index of the first record at or after start_time.
local function find_first_valid_index(values, start_time)
...
function FS.modules.heal_engine.get_damage_taken_per_second(unit, last_x_seconds)
    ...
    local first_valid_index = find_first_valid_index(values, start_time)
